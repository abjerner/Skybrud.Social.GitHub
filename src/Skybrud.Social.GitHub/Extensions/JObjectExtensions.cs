...
namespace Skybrud.Social.GitHub.Extensions {
    ...
    public static class JObjectExtensions {
        ...
        /// <paramref name="propertyName"/>. If the property is not found, or it's value is empty, <see langword="null"/> is
        /// returned instead.
        /// </summary>
        ...
        public static EssentialsTime? GetEssentialsTime(this JObject json, string propertyName) {
            ...
            return json.GetString(propertyName, EssentialsTime.Parse);
        }

        /// <summary>
        /// Gets an instance of <see cref="EssentialsTime"/> from the token at the specified <paramref name="path"/>.
        /// If a token a matching token is not found, or it's value is empty, <see langword="null"/> will be returned instead.
        /// </summary>
        /// <param name="json">The JSON object.</param>
        ...
        public static EssentialsTime? GetEssentialsTimeByPath(this JObject json, string path) {
            ...
            return json.GetStringByPath(path, EssentialsTime.Parse);
        }

        internal static EssentialsTime GetRequiredEssentialsTime(this JObject json, string propertyName) {
            if (string.IsNullOrWhiteSpace(propertyName)) throw new ArgumentNullException(nameof(propertyName));
            return EssentialsTime.FromIso8601(json.GetRequiredString(propertyName));
        }
